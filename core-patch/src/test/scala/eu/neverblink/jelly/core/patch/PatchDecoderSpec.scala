...
class PatchDecoderSpec extends AnyWordSpec, Matchers:
  ...
  for (decName, decFactory, stType) <- decoders do
    decName should {
      "throw exception on overly large name lookup table" in {
        val input = rdfPatchFrame(
          Seq(
            rdfPatchRow(
      ...
      for (st, i) <- streamTypes do
        f"accept stream with type $st" in {
      ...
      for (st, i) <- streamTypes do
        f"reject stream with type $st if a different stream type is expected" in {
          val opt = JellyPatchOptions.SMALL_STRICT.clone
            .setStatementType(stType)
            .setStreamType(st)
      ...
      for (st, _) <- streamTypes.take(2) do
        f"throw exception on punctuations in stream type $st" in {
          val input = rdfPatchFrame(
            Seq(
              rdfPatchRow(
                JellyPatchOptions.SMALL_STRICT.clone
