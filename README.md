...
The `jelly-stream` and `jelly-grpc` currently use the Akka library (2.6.x), with the last release available on the Apache 2.0 license. Due to the licensing change of Akka, in future versions it is planned to switch to the community fork of Akka – [Apache Pekko](https://github.com/apache/incubator-pekko), when it becomes available.
...
- `jelly-stream` – utilities for building [Reactive Streams](https://www.reactive-streams.org/) of RDF data (based on Akka Streams). Useful for integrating with gRPC or other streaming protocols (e.g., Kafka, MQTT).
