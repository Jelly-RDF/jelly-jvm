...
object RdfStreamServiceClient {
  ...
  private class DefaultRdfStreamServiceClient(channel: GrpcChannel, isChannelOwned: Boolean)(implicit sys: ClassicActorSystemProvider) extends RdfStreamServiceClient {
     ...
     * prefer subscribeRdf(RdfStreamSubscribe) if possible.
     */
    ...
    /**
     * Lower level "lifted" version of the method, giving access to request metadata etc.
     * prefer publishRdf(Source[RdfStreamFrame, NotUsed]) if possible.
     */

    override def publishRdf(): SingleResponseRequestBuilder[Source[RdfStreamFrame, NotUsed], RdfStreamReceived] =
     ...
     */
    def publishRdf(in: Source[RdfStreamFrame, NotUsed]): scala.concurrent.Future[RdfStreamReceived] =
...
trait RdfStreamServiceClientPowerApi {
   ...
   * prefer publishRdf(Source[RdfStreamFrame, NotUsed]) if possible.
   */
  def publishRdf(): SingleResponseRequestBuilder[Source[RdfStreamFrame, NotUsed], RdfStreamReceived] = ???
}
