...
object RdfStreamServiceHandler {
  ...
  private val unsupportedMediaType = Future.successful(model.HttpResponse(model.StatusCodes.UnsupportedMediaType))
   ...
   */
  def apply(implementation: RdfStreamService, prefix: String)(implicit system: ClassicActorSystemProvider): model.HttpRequest => Future[model.HttpResponse] =
   ...
   * Registering a gRPC service under a custom prefix is not widely supported and strongly discouraged by the specification.
   */
  def apply(implementation: RdfStreamService, prefix: String, eHandler: ActorSystem => PartialFunction[Throwable, Trailers])(implicit system: ClassicActorSystemProvider): model.HttpRequest => Future[model.HttpResponse] =
   ...
   * Registering a gRPC service under a custom prefix is not widely supported and strongly discouraged by the specification.
   */
  def partial(implementation: RdfStreamService, prefix: String = RdfStreamService.name, eHandler: ActorSystem => PartialFunction[Throwable, Trailers] = GrpcExceptionHandler.defaultMapper)(implicit system: ClassicActorSystemProvider): PartialFunction[model.HttpRequest, Future[model.HttpResponse]] = {
