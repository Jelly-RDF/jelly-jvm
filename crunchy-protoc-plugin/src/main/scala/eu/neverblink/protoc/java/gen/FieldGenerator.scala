...
/** This class generates all serialization logic and field related accessors. It is a bit of a mess
  * due to lots of switch statements, but I found that splitting the types up similarly to how the
  * protobuf-generator code is organized makes it really difficult to find and manage duplicate
  * code, and to keep track of where things are being called.
  *
  * @author
  *   Florian Enner
  * @author
...
class FieldGenerator(val info: FieldInfo):
  ...
  m.put(
    "valueOrNumber",
    if (info.isEnum) "value.getNumber()"
    else "value",
  )
  ...
  if (info.isRepeated)
    m.put("getRepeatedIndex_i", if (info.isPrimitive || info.isEnum) "array()[i]" else "get(i)")
  // utility classes
  ...
  def generateMemberFields(t: TypeSpec.Builder): Unit =
    ...
    if info.isRepeated || info.isBytes || info.isString then field.initializer(initializer)
  ...
  private def initializer =
    ...
    if (info.isRepeated && info.isMessageOrGroup)
      initializer.add(
        "new $T($T::newInstance)",
        RuntimeClasses.ListMessageCollection,
        info.getTypeName,
      )
    else if (info.isRepeated && info.isEnum)
      initializer.add(
        "$T.newEmptyInstance($T.converter())",
        RuntimeClasses.RepeatedEnum,
        info.getTypeName,
      )
  ...
  def generateClearCode(method: MethodSpec.Builder): Unit =
    if info.isSingularPrimitiveOrEnum then method.addStatement(named("$field:N = $default:L"))
    else if info.isRepeated then method.addStatement(named("$field:N.clear()"))
    else if info.isMessageOrGroup then method.addStatement(named("$field:N = null"))
    else if info.isString then method.addStatement(named("$field:N = \"\""))
    else if info.isBytes then method.addStatement(named("$field:N = ByteString.EMPTY"))
    else throw new IllegalStateException("unhandled field: " + info.descriptor)
  ...
  def generateMergingCode(method: MethodSpec.Builder): Boolean =
    ...
    if (info.isRepeated && info.isMessageOrGroup) {
      method.addStatement(
        "tag = $T.readRepeatedMessage($N, input, tag, remainingDepth)",
  ...
  private def generateInitializedMethod(t: TypeSpec.Builder): Unit =
    if !info.isRepeated && info.isMessage then
      t.addMethod(
        MethodSpec.methodBuilder(info.lazyInitName)
  ...
  private def generateSetMethods(t: TypeSpec.Builder): Unit =
    if (info.isBytes) {
      ...
      t.addMethod(
        MethodSpec.methodBuilder(info.setterName)
          .addJavadoc(
            Javadoc.forMessageField(info)
              .add("\n@param value the $L to set", info.fieldName)
              .add("\n@return this")
  ...
  /** Enums are odd because they need to be converter back and forth and they don't have the same
    * type as the internal/repeated store. The normal accessors provide access to the enum value,
    * but for performance reasons we also add accessors for the internal storage type that do not
    * require conversions.
    *
    * @param t
    */
  private def generateExtraEnumAccessors(t: TypeSpec.Builder, tMutable: TypeSpec.Builder): Unit = {
    ...
    t.addMethod(
      MethodSpec.methodBuilder(info.getterName + "Value")
        .addAnnotations(info.methodAnnotations)
        .addJavadoc(
          named(
            "" +
    ...
    tMutable.addMethod(
      MethodSpec.methodBuilder(info.setterName + "Value")
        .addAnnotations(info.methodAnnotations)
        .addJavadoc(
          named(
            "" +
