...
//
//import eu.neverblink.jelly.core.patch.{JellyPatchOptions, PatchConstants}
...
//    (Some(JellyPatchOptions.SMALL_STRICT.copy(streamType = FLAT)), 100, "small strict, FLAT"),
//    (Some(JellyPatchOptions.SMALL_RDF_STAR.copy(streamType = FRAME)), 100, "small RDF-star, FRAME"),
//    (Some(JellyPatchOptions.SMALL_GENERALIZED.copy(streamType = FRAME)), 10, "small generalized, FRAME"),
//    (Some(JellyPatchOptions.SMALL_ALL_FEATURES.copy(streamType = PUNCTUATED)), 1000, "small all features, PUNCTUATED"),
//    (Some(JellyPatchOptions.BIG_STRICT.copy(streamType = PUNCTUATED)), 5, "big strict, PUNCTUATED"),
//    (Some(JellyPatchOptions.BIG_RDF_STAR.copy(streamType = FLAT)), 5, "big RDF-star, FLAT"),
//    (Some(JellyPatchOptions.BIG_GENERALIZED.copy(streamType = PUNCTUATED)), 5, "big generalized, PUNCTUATED"),
//    (Some(JellyPatchOptions.BIG_ALL_FEATURES.copy(streamType = FRAME)), 5, "big all features, FRAME"),
//    (None, 10, "no options"),
//  )
//
...
//    expectedPatches: Int,
//  ): Unit =
//    val expOpt = expectedOpt.getOrElse(JellyPatchOptions.BIG_ALL_FEATURES.copy(
...
//    frames.head.getRows.asScala.head.rowType should be (RdfPatchRow.OPTIONS_FIELD_NUMBER)
//    val opt = frames.head.getRows.asScala.head.asInstanceOf[RdfPatchOptions]
...
//    // Check punctuation
//    val punctMarks = frames.flatMap(_.getRows.asScala).count(_.rowType == RdfPatchRow.PUNCTUATION_FIELD_NUMBER)
