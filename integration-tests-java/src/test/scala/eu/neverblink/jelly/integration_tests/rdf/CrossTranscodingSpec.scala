...
//    (frame: RdfStreamFrame) => frame.getRows.asScala.flatMap(decoder.ingestRow)
//
//  def addStreamTypeToOptions(opt: RdfStreamOptions, t: String): RdfStreamOptions =
//    if t == "triples" then
//      opt.setPhysicalType(PhysicalStreamType.TRIPLES).withLogicalType(LogicalStreamType.FLAT_TRIPLES)
//    else if t == "quads" then
//      opt.setPhysicalType(PhysicalStreamType.QUADS).withLogicalType(LogicalStreamType.FLAT_QUADS)
//    else
//      opt.setPhysicalType(PhysicalStreamType.GRAPHS).withLogicalType(LogicalStreamType.NAMED_GRAPHS)
//
...
//    ("small, no prefix table", JellyOptions.SMALL_ALL_FEATURES.setMaxPrefixTableSize(0)),
//    (
//      "tiny name table, tiny prefix table",
//      JellyOptions.SMALL_ALL_FEATURES.setMaxPrefixTableSize(8).setMaxNameTableSize(16)
//    ),
