...
/** Base trait for builders of RDF stream sources.
  * @tparam TTotal
  *   Total type of this source stage (including the child stage).
  * @tparam TApp
...
final class RdfSourceBuilderImpl[TGraph, TDataset, TNode, TTriple, TQuad]:
  ...
  /** Create a new root RDF source builder. Internal API. Use [[RdfSource.builder]] instead.
    * @return
    *   root builder
    */
  ...
  sealed trait RootBuilder:
    /** Turn an RDF graph into a stream of triples.
      ...
      *   RDF graph
      * @param adapter
      *   converter to convert the graph into a stream of triples and the original graph type
      *
      * @return
      *   source builder that can be materialized into a Pekko Source or further extended
      */
    final def graphAsTriples(graph: TGraph)(using
        adapter: GraphAdapter[TTriple, TGraph],
    ): ExtensibleBuilder[TTriple, Nothing, TGraph] =
      new ExtensibleBuilder[TTriple, Nothing, TGraph](None, graph):
        protected def sourceInternal: Source[TTriple, NotUsed] = Source(
          adapter.triples(graph).asScala.toList,
        )
    ...
    /** Turn an RDF dataset into a stream of quads.
      ...
      *   RDF dataset
      * @param adapter
      *   converter to convert the dataset into a stream of quads and the original dataset type
      *
      * @return
      *   source builder that can be materialized into a Pekko Source or further extended
      */
    final def datasetAsQuads(dataset: TDataset)(using
        adapter: DatasetAdapter[TNode, TTriple, TQuad, TDataset],
    ): ExtensibleBuilder[TQuad, Nothing, TDataset] =
      new ExtensibleBuilder[TQuad, Nothing, TDataset](None, dataset):
        protected def sourceInternal: Source[TQuad, NotUsed] = Source(
          adapter.quads(dataset).asScala.toList,
        )
    ...
    /** Turn an RDF dataset into a stream of (node, graph) pairs.
      ...
      *   RDF dataset
      * @param adapter
      *   converter to convert the dataset into a stream of (node, graph) pairs and the original
      *   dataset type
      *
      * @return
      *   source builder that can be materialized into a Pekko Source or further extended
      */
    final def datasetAsGraphs(dataset: TDataset)(using
        adapter: DatasetAdapter[TNode, TTriple, TQuad, TDataset],
  ...
  sealed trait ExtensibleBuilder[TApp, TChild, TSrc <: TGraph | TDataset](
      _child: Option[RdfSourceBuilder[TChild, ?, ?]],
      src: TSrc,
  ) extends RdfSourceBuilder[TApp | TChild, TApp, TChild]:
    ...
    /** Mix in namespace declarations into the stream.
      ...
      *
      * @param converter
      *   converter to convert the source into a stream of namespace declarations and the original
      *   source type
      * @return
      *   source builder that can be materialized into a Pekko Source or further extended
      */
    final def withNamespaceDeclarations(using
        converter: NamespaceAdapter[TSrc],
