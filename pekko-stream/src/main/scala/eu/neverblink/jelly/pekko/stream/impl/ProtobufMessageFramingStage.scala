...
class ProtobufMessageFramingStage(val maxMessageSize: Int)
  ...
  override def createLogic(inheritedAttributes: Attributes): GraphStageLogic =
    new GraphStageLogic(shape) with InHandler with OutHandler {
       ...
       *
       * Will store the decoded message size in `messageSize` and the length of the
       * varint in `lengthSize`.
       *
       * @param iterator Iterator of bytes to read from
       * @return true if a complete varint was read, false if not enough bytes were available
       */
