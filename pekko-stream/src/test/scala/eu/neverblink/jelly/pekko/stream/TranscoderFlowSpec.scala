...
/** Basic functionality tests for the TranscoderFlow class. This really only tests if the flows are
  * working as intended, not the transcoder itself. For that, see tests in the core and
    ...
    "transcode triples" when {
      ...
      "rowToRow" in {
        Source(Triples1.encodedFull(JellyOptions.SMALL_ALL_FEATURES, 4).flatMap(_.getRows.asScala))
          ...
          Triples1.encodedFull(JellyOptions.SMALL_ALL_FEATURES, 4).flatMap(_.getRows.asScala),
        )
      }
      ...
      "rowToFrame" in {
        Source(Triples1.encodedFull(JellyOptions.SMALL_ALL_FEATURES, 4).flatMap(_.getRows.asScala))
          .via(
            TranscoderFlow.fastMergingUnsafe(JellyOptions.SMALL_ALL_FEATURES).rowToFrame(
              StreamRowCountLimiter(4),
            ),
          )
          .runWith(Sink.seq)
          .futureValue should be(Triples1.encodedFull(JellyOptions.SMALL_ALL_FEATURES, 4))
    ...
    "transcode quads" when {
      ...
      "rowToRow" in {
        Source(Quads1.encodedFull(JellyOptions.SMALL_ALL_FEATURES, 5).flatMap(_.getRows.asScala))
          ...
          .runWith(Sink.seq)
          .futureValue should be(
          Quads1.encodedFull(JellyOptions.SMALL_ALL_FEATURES, 5).flatMap(_.getRows.asScala),
        )
      }
        ...
        Source(Quads1.encodedFull(JellyOptions.SMALL_ALL_FEATURES, 5).flatMap(_.getRows.asScala))
          .via(
            TranscoderFlow.fastMerging(
              JellyOptions.DEFAULT_SUPPORTED_OPTIONS,
              JellyOptions.SMALL_ALL_FEATURES,
            ).rowToFrame(StreamRowCountLimiter(5)),
          )
          .runWith(Sink.seq)
          .futureValue should be(Quads1.encodedFull(JellyOptions.SMALL_ALL_FEATURES, 5))
      }
