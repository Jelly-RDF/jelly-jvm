...
class CrossPatchSpec extends AnyWordSpec, Matchers:
  ...
  val presets: Seq[(Option[RdfPatchOptions], Int, String)] = Seq(
    ...
    (
      Some(JellyPatchOptions.SMALL_RDF_STAR.clone().setStreamType(FRAME)),
      100,
      "small RDF-star, FRAME",
    ),
    (
      Some(JellyPatchOptions.SMALL_GENERALIZED.clone().setStreamType(FRAME)),
      10,
      "small generalized, FRAME",
    ),
    (
      Some(JellyPatchOptions.SMALL_ALL_FEATURES.clone().setStreamType(PUNCTUATED)),
      1000,
      "small all features, PUNCTUATED",
    ),
    (
      Some(JellyPatchOptions.BIG_STRICT.clone().setStreamType(PUNCTUATED)),
      5,
      "big strict, PUNCTUATED",
    ),
    (Some(JellyPatchOptions.BIG_RDF_STAR.clone().setStreamType(FLAT)), 5, "big RDF-star, FLAT"),
    (
      Some(JellyPatchOptions.BIG_GENERALIZED.clone().setStreamType(PUNCTUATED)),
      5,
      "big generalized, PUNCTUATED",
    ),
    (
      Some(JellyPatchOptions.BIG_ALL_FEATURES.clone().setStreamType(FRAME)),
      5,
      "big all features, FRAME",
    ),
    (None, 10, "no options"),
  )
  ...
  private def checkOptionsAndPunctuation(
    ...
    // Check punctuation
    val punctMarks =
      frames.flatMap(_.getRows.asScala).count(_.getRowFieldNumber == RdfPatchRow.PUNCTUATION)
    if expOpt.getStreamType == FRAME then
      frames.size should be(expectedPatches)
      punctMarks should be(0)
    else if expOpt.getStreamType == PUNCTUATED || expOpt.getStreamType == PatchStreamType.UNSPECIFIED
    then punctMarks should be(expectedPatches)
    else // FLAT
      punctMarks should be(0)
  ...
  private def runTest[T1: TestComparable, T2: TestComparable](
    ...
    f"${impl1.name} serializer + ${impl2.name} deserializer" should {
      for
        (name, files) <- TestCases.cases.filter(tc =>
          impl1.supportsRdfStar && impl2.supportsRdfStar || !tc._2.exists(f =>
            f.getName.contains("star"),
          ),
        )
        (preset, size, presetName) <- presets
      do
        f"ser/des file $name with preset $presetName, frame size $size" when {
          for statementType <- Seq(TRIPLES, QUADS)
          do
            f"statement type $statementType" in {
              ...
              // stream to be QUADS.
              if c1 == c2 && opt.isDefined then c1.compare(m1, m2.asInstanceOf[T1])
              ...
              if opt.isDefined then c1.compare(m1, m1_b)
            }
        }
    }
