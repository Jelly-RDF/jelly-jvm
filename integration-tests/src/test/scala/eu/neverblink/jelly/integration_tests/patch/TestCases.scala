...
object TestCases:
  val cases: Seq[(String, Seq[File])] = Seq[(String, Seq[String])](
    ...
    (
      "triples-all-features-rdf11 (repeated x5)",
      (0 to 5).map(_ => "triples-all-features-rdf11.rdfp"),
    ),
    ("quads-all-features-rdf11", Seq("quads-all-features-rdf11.rdfp")),
    (
      "quads + triples + quads-all-features-rdf11",
      Seq(
    ...
    (
      name,
      filenames.map(n => File(getClass.getResource("/patch/" + n).toURI)),
    ),
  )
